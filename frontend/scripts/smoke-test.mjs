const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5510';
...
const adminCookie = await login('admin@property-hoa-management.local', 'admin123');
const managerCookie = await login('manager@property-hoa-management.local', 'manager123');
const analystCookie = await login('analyst@property-hoa-management.local', 'analyst123');
